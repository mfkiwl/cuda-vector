...
struct LFVector {
	...
	T **a;
	int *isbucket;
	__device__ LFVector(MemoryManagerType *mm);
...
template <typename T>
__device__ LFVector<T>::LFVector(MemoryManagerType *mm) {
	this->mm = mm;
	size = 0;
...
__device__ void LFVector<T>::resize(unsigned int n) {
	...
	for (int i = b1+1; i <= b2; ++i) {
		...
		a[i] = reinterpret_cast<T*>(mm->malloc(sizeof(T) * bsize));
		isbucket[i] = 1;
	}
	size = n;
}
...
__device__ void LFVector<T>::new_bucket(unsigned int b) {
	...
	if (old == 0) {
		...
		a[b] = reinterpret_cast<T*>(mm->malloc(sizeof(T) * bsize));
	}
	__syncthreads();
}
...
struct Vector {
	...
	LFVector<T> *lfv;
	unsigned int *ranges;
	__device__ Vector(MemoryManagerType *mm);
	__device__ T& at(unsigned int i);
...
template <typename T>
__device__ Vector<T>::Vector(MemoryManagerType *mm) {
	this->mm = mm;
	size = 0;
	//lfv = (LFVector<T>*)malloc(sizeof(LFVector<T>)*NB);
	lfv = reinterpret_cast<LFVector<T>*>(mm->malloc(sizeof(LFVector<T>)*NB));
	//ranges = (unsigned int*)malloc(sizeof(unsigned int)*NB);
	ranges = reinterpret_cast<unsigned int*>(mm->malloc(sizeof(unsigned int)*NB));
	for (int i = 0; i < NB; ++i) {
		ranges[i] = 0;
		lfv[i] = LFVector<T>(mm);
	}
...
template <typename T>
__global__ void createLFVector(Vector<T> *v, MemoryManagerType *mm) {
	*v = Vector<T>(mm);
	return;
	v->size = 0;
	for (int i = 0; i < NB; ++i) {
...
void run_experiment(Vector<int> *v, int size, int ratio, MemoryManagerType mm) {
	int rep = 10;
	int rw_rep = 30;
	int o_size = size;
	createLFVector<<<1,1>>>(v, mm.getDeviceMemoryManager()); kernelCallCheck();
...
int main(int argc, char **argv){
	...
	run_experiment(lfv, size, 2, memory_manager);
